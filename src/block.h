...
public:
	// Constructor: Arguments are the block size and (optionally) the pointer to the function which
	// will be called if allocation failed; the message passed to this function is "Not enough memory!"
	...
	// Allocates 'num' consecutive items; returns pointer to the first item. 'num' cannot be greater than the
	...
	// Returns the next item (or NULL, if all items have been read) Can be called only if previous ScanFirst() or ScanNext()
