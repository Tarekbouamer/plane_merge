...
// If it is connected to the sink, it stays in the list, otherwise it is removed from the list
template <typename captype, 
					typename tcaptype, 
					typename flowtype> 
...
void Graph<captype,tcaptype,flowtype>::process_source_orphan(node *i)
{
	...
	if (a0->sister->r_cap){
		...
		if (!j->is_sink && (a=j->parent)){
			...
			d = 0;
			while (1) {
				if (j->TS == TIME){	d += j -> DIST;	break;	}
	
				a = j -> parent;
				d ++;
				
				if (a==TERMINAL){ j -> TS = TIME;	j -> DIST = 1;	break;	}
