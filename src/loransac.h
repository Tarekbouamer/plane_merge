...
LORANSAC<Estimator, LocalEstimator, SupportMeasurer, Sampler>::Estimate(const std::vector<typename Estimator::X_t>& X, const std::vector<typename Estimator::Y_t>& Y) {
  ...
  if (report.support.num_inliers < estimator.kMinNumSamples) { return report; }
